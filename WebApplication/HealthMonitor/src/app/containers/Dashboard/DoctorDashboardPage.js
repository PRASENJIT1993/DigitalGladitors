...
import { DoctorDash } from '../../presentations/Dashboard/DoctorDash';
import { fetchData, postData } from '../../actions/Action';
import { getPatientGraphStatus } from '../../utils/Seletors';
...
class DoctorDashboardPage extends React.Component {
  ...
  componentWillMount() {
    this.props.actionHandler(postData(ActionConstants.GET_PATIENT_LIST_BY_ID, {
  ...
  render() {
    return (
      <div>
        <DoctorDash {...this.props} data={this.props.pateientGraphStatus} />
      </div>
    );
  }
}
...
function mapStateToProps(state) {
  return {
    ...
    pateientGraphStatus: getPatientGraphStatus(state),
...
export default connect(mapStateToProps, mapDispatchToProps)(withWidth()(DoctorDashboardPage));